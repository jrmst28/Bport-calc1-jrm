...
=======
        ...
        if a==2:
            flatex = LatexExpr(f"{c}\\cdot \\sqrt{{{x}}}")
            dflatex = LatexExpr(f"{c}\\cdot \\frac{{1}}{{2\\sqrt{{{x}}}}}")
        else:   
            flatex = LatexExpr(f"{c}\\cdot \\sqrt[{a}]{{{x}}}")
            dflatex = LatexExpr(f"({c})(\\frac{{1}}{{{a}}}) {x}^{{{anew}}}")
        fs += [
            {
                "f":flatex,
                "fn":fn,
        ...
        if a==2:
            ...
            flatex = LatexExpr(f"\\frac{{{c}}}{{\\sqrt[{a}]{{{x}}}}}")
        dflatex = LatexExpr(f"({c})(\\frac{{-1}}{{{a}}}) {x}^{{-{a+1}/{a}}}")
        fs += [
            {
                "f":flatex,
                "fn":fn,
        ...
        if a==2:
            ...
            flatex = LatexExpr(f"{c}\\cdot \\sqrt[{a}]{{{x}^{b}}}")
        dflatex = LatexExpr(f"({c})(\\frac{{{b}}}{{{a}}}) {x}^{{{b-a}/{a}}}")
        fs += [
            {
                "f":flatex,
                "fn":fn,
...
>>>>>>> b76f3bc1a3a0ea8f6e09ec4bcb2cabfafe235adb
        #negative power
        x=choice(xs)
        fn=choice(fns)
